...
void displayInt(const int &n) { std::cout << n << " "; }
...
void test6_null_function() {
  ...
  std::cout << "\033[1;32mSUCCESS: No crash on NULL function pointer!\033[0m"
            << std::endl;
}
...
void test7_const_null_hint() {
  ...
  std::cout << "Testing NULL pointer with 'const int&' parameter..."
            << std::endl;
  ::iter(intArray, 3, nullConstFunc);
  std::cout
      << "\033[1;32mSUCCESS: Const NULL caught by specialized template!\033[0m"
      << std::endl;
}
